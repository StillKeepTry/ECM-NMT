...
class TransformerDecoder(FairseqIncrementalDecoder):
        ...
        **extra_args
        ...
        if incremental_state is not None:
            prev_output_tokens = torch.cat((prev_output_tokens, 
                                            prev_output_tokens.new_full((prev_output_tokens.size(0), 1), self.mask_idx)), dim=-1)

        max_iter = 0
        if 'max_iter' in extra_args:
        ...
        if False:
            _, nx = x.topk(1, dim=-1)
            prev_output_tokens[:, -1] = nx.squeeze()
    ...
    def extract_features(
        ...
        **unused,
        ...
        # embed positions
        _ = self.embed_positions(
            prev_output_tokens,
            incremental_state=incremental_state,
        ) if self.embed_positions is not None else None

        if incremental_state is not None:
            s = 2 if update_cache else 1
            prev_output_tokens = prev_output_tokens[:, -s:]
            positions = torch.arange(sz[1])[-s:].to(prev_output_tokens) + 2
            positions = F.embedding(positions, self.embed_positions.weights, self.padding_idx)
        ...
        return x[:, -1:], {'attn': attn, 'inner_states': inner_states}
